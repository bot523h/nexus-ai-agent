...
| R13 | Apply-lane lifecycle is gates-only (`lifecycle.py` has no `subprocess`); encode requires RUNNABLE; `executor.py` is the only process site; AST ratchet detects a smuggled `subprocess.run` | `test_rendering_lane_boundary.py::test_exactly_one_subprocess_site_and_no_shell_true`, `::test_lifecycle_module_is_gates_only`, `::test_ratchet_detects_subprocess_smuggled_into_lifecycle`, `tests/unit/test_apply_lane_lifecycle.py` |
