...
| `creative/studio/` | L1 | Nagar core: typed `Project`, `CommandBus`, `CapabilityRegistry`, `PermissionLevel`, reference resolver, discovery surface + preview/master stamps | `build_wave1_registry`, `CommandBus.dispatch`, `build_capability_surface`, `error_code_of`, `is_master_evidence` |
...
| R14 | The discovery surface is a fail-closed projection of runtime truth: nothing is advertised unless its capability is available **and** every required pack resolves to an executable lifecycle state under the composition flag; exclusions carry typed reason codes and no permission data; the surface is deterministic (byte-stable canonical JSON) | `test_studio_discovery.py` (Law-4 projection per lifecycle state, determinism + identity, error-contract codes, exclusion field ratchet) |
| R15 | The preview/master boundary is declared at registration and stamped at dispatch: advertising `preview` without `preview_semantics` is refused (and orphaned semantics are refused); every `CommandResult` carries `execution_mode` + `authoritative`; only `authoritative is True` results are master evidence, unstamped results fail closed | `test_studio_preview_master_boundary.py` (both registration refusals, stamp matrix, failed-execution forgery guard, pinned baseline), `test_studio_integration_contracts.py::TestAgent3RuntimeContract` |
