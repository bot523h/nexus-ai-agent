...
| T15 | **Information disclosure (SSRF)** | a caller-supplied `video_url` on the legacy `POST /creative/video-edit` makes the server fetch loopback/link-local/private targets (found live on `main`) | canonical `core/ssrf_guard.py` in the request path (400 before any job row), `SafeAsyncTransport` re-validating every connect and every redirect hop, bounded read + timeout, media-type allow-list | `tests/unit/test_legacy_creative_security.py::test_video_url_never_reaches_loopback` (real listener), `tests/architecture/test_creative_single_path.py` | closed (2026-09-24) |
| T16 | **Elevation / information disclosure (IDOR)** | `GET /creative/jobs/{job_id}` answered 200 to anyone, including `input_data` (staged paths, source URLs) and error text | fail-closed HMAC authn on both legacy routes, principal = API-key holder, minimized `_public_job_view` projection, identical 404 for unknown ids | `tests/unit/test_legacy_creative_security.py`, `tests/unit/test_creative_studio.py` | closed (2026-09-24) |
