...
| T8 | **Information disclosure** | secrets in logs | redaction at the observability boundary in *both* pipelines (stdlib filter + structlog processor, and the lifecycle `redact_fields` gate): bearer values, `token=`/`api_key=`/`password=`, bare `?key=`/`?token=` query params, `x-goog-api-key` headers, bare and API-URL bot tokens, URL userinfo, secret-ish keys in nested structures — asserted on captured rendered log output | `tests/unit/test_observability.py`, `tests/unit/test_structured_events.py` | closed |
| T9 | **Information disclosure** | SSRF via summariser/WebTrainer/image URLs and the legacy `video_url` download | DNS/address validation + validating transport (private ranges refused): fail-fast `validate_url` at job creation **and** `SafeAsyncTransport` at fetch time — every connection incl. redirect hops is re-resolved, re-checked and IP-pinned (DNS-rebinding TOCTOU closed) | `tests/unit/test_http_client_ssrf.py`, `tests/unit/test_summarizer_ssrf.py`, `tests/unit/test_api_ssrf_download.py` | closed |
