...
| T11 | **Tampering (supply chain)** | a pack ships executable code, an unknown operation, or an unverified external signature | data-only manifests (no executable key at any depth), external packs cannot register unknown operations, activation refuses pending capabilities **and refuses every external pack: no Ed25519 verifier or publisher key chain exists, so `TRUSTED_SIGNATURE_STATES` is empty and no emittable signature state grants activation trust** (PACK-SEC-001; this is fail-closed refusal, *not* cryptographic verification) | `tests/architecture/test_pack_manifest_is_data_only.py`, `tests/unit/test_pack_manifest_verify.py::test_external_pack_with_unverified_signature_cannot_activate` (+ builtin control arm, `test_no_state_the_current_verifier_can_emit_is_trusted`; mutation-proved in task-191) | closed (refusal); signature verification **not implemented** |
