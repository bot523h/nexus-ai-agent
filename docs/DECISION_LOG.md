...
- **PR#33** — “v3.13.0 — deliver the P0 security code, wire the dead engines, fix 4 production bugs” (`arena/01a0c3aa`): **CLOSED 2026-09-21 as superseded** — its security scope landed through merged PR#34 (`93cee5e`), its head was `CONFLICTING` with `main`, and its feature-wiring portion overlaps agent B's active `feature-wiring` lease (PR#32). Head branch preserved for archaeology; the unique ConversationStorePort-adapter idea remains tracked as task-110.
