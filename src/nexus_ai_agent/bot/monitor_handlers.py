...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text("❌ استفاده: /approve <id>")
        return
    ...
    try:
        ...
        if success:
            await message.reply_text(f"✅ درخواست {approval_id} تایید شد.")
        else:
...
async def reject_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await message.reply_text("❌ استفاده: /reject <id>")
        return
    ...
    try:
        ...
        if success:
            await message.reply_text(f"❌ درخواست {approval_id} رد شد.")
        else:
            msg = f"❌ درخواست {approval_id} یافت نشد یا قبلاً بررسی شده است."
            await message.reply_text(msg)
    except ValueError:
