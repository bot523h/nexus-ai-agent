...
def _as_utc(value: datetime | None) -> datetime | None:
    """SQLite hands back naive datetimes; treat them as UTC (they were stored as UTC)."""
    if value is None:
        return None
...
class GamificationEngine:
    ...
    def claim_daily(user_id: int, chat_id: int) -> dict[str, Any]:
        """Claim the daily XP reward. Returns reward info.

        Runs in a single session: the previous version called
        :meth:`update_streak` (a second engine/connection) while holding an
        uncommitted INSERT, which deadlocked SQLite on a user's very first
        claim.  Naive ``last_daily`` values from SQLite are treated as UTC.
        """
        now = datetime.now(timezone.utc)
        engine = _sync_engine()
        with Session(engine) as session:
            ...
            streak_broken = False
            if last_daily is not None:
                diff_days = (now - last_daily).total_seconds() / 86400
                if diff_days < 1.0:
                    remaining = 1.0 - diff_days
                    return {
                        "claimed": False,
