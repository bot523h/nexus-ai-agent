...
* ``EXECUTABLE`` has a lane twin in
  ...
  provided the engine binary ships the twin's filters
  (:data:`TWIN_ENGINE_FILTERS` — e.g. ``drawtext`` is absent from the
  imageio-ffmpeg wheel, so titles need a libfreetype-enabled binary).
...
#: Lane twin → the FFmpeg filters its stage needs.  Session-3 honesty note:
#: the suite's default engine (the imageio-ffmpeg wheel) ships ``lut3d``,
#: ``subtitles``/``ass``, and ``eq``/``colortemperature``/``colorbalance``
#: but NOT ``drawtext`` (no libfreetype) — so the title twin renders real
#: pixels only where a drawtext-capable binary resolves (system FFmpeg on
#: CI).  The drawtext proof test skips loudly elsewhere instead of faking it.
TWIN_ENGINE_FILTERS: dict[str, tuple[str, ...]] = {
    "exposure": ("eq", "colortemperature", "colorbalance"),
    "title": ("drawtext",),
    "lut": ("lut3d",),
    "subtitle": ("subtitles",),
}
...
def classify_operation(operation_id: str, *, registry: Any = None) -> ExecutionRecord:
    ...
    if twin is not None:
        try:
            need = TWIN_ENGINE_FILTERS[twin]
        except KeyError:
            raise ValueError(
                f"lane twin {twin!r} has no engine-filter entry in TWIN_ENGINE_FILTERS"
            ) from None
        filters = ", ".join(need)
        return ExecutionRecord(
            ...
            notes=(
                f"lane twin {twin!r}: plan → LaneIR → FFmpeg renders real pixels "
                f"(engine must provide: {filters})"
            ),
        )
