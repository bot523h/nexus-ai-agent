...
def _compile(
    ...
    *,
    ...
    options = _ChainOptions(
        fps=profile.fps, fontfile=fontfile, measured=measured, measure_mode=measure_mode
    )
...
def compile_assembly(assembly: LaneAssembly, *, fontfile: str | None = None) -> CompiledLane:
    ...
    * an ``exposure`` op in an audio-only assembly (same rule as single-lane).
    """
    ...
    for segment in segments:
        ...
        if not has_video and any(isinstance(op, ExposureOp) for op in segment.ops):
            raise LaneError("exposure op needs a video main asset")

    profile = assembly.profile
    normalize = _normalize_video(profile.width, profile.height, profile.fps)
    options = _ChainOptions(fps=profile.fps, fontfile=fontfile, measured=None, measure_mode=False)
