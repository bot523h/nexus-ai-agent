...
Multi-segment tracks compile through :func:`assemble_execution_plan` into one
:class:`LaneAssembly` (ordered segments + editorial gaps), then
:func:`compile_assembly` emits one filtergraph with one ``concat`` stage for
the *same* executor — no second pipeline.  ``concat_required=True`` on a plan
now means "assemble, then render".

Determinism contract: same project + same track (+ same staged-asset maps) →
identical segments and identical ``plan_hash`` (canonical JSON, sorted keys,
integer microseconds).  The input project is never mutated.
"""
...
#: Session 2 mapped exposure; session 3 adds the title/LUT/subtitle twins
#: (task-168 mappings that need no new ``LaneOp`` plus the two new lane
#: instruments).  ``execution.py`` derives EXECUTABLE from this map — never
#: hand-claim an entry here without a lane twin and a real-artifact test.
...
def _effect_params(effect: Any) -> dict[str, Any]:
    ...
    if not isinstance(params, dict):
        raise PlanError(f"effect {effect.operation!r} carries non-dict parameters")
    return dict(params)
...
def _map_effect(
    ...
    *,
    clip_id: str,
    lut_paths: dict[str, str] | None,
    subtitle_files: dict[str, str] | None,
) -> tuple[object | None, str | None]:
    """Map one ``EffectLayerRef`` onto a fresh lane op, or report it unmapped.

    Every call constructs new op objects — no shared mutable state crosses
    segments (E1: segment compilation is referentially isolated).  A twin
    whose *asset* is missing (LUT name, staged caption file) is a
    :class:`PlanError` naming the clip, never a silent drop or a fake op.
    """
    ...
    try:
        ...
        if twin == "title":
            # Title timings are segment-relative (the trim resets PTS first),
            # so the effect range is NOT reinterpreted as drawtext enable
            # bounds — explicit start/end win, otherwise the whole segment.
            text = params.get("text")
            if not isinstance(text, str) or not text:
                raise PlanError(f"clip {clip_id!r}: motion.add_title needs a text parameter")
            end_us = params.get("end_us")
            return (
                TitleOp(
                    text=text,
                    ...
                    start_us=int(params.get("start_us", 0)),
                    end_us=None if end_us is None else int(end_us),
                ),
                None,
            )
        if twin == "lut":
            lut_name = params.get("lut_name")
            if not isinstance(lut_name, str) or not lut_name:
                raise PlanError(f"clip {clip_id!r}: color.apply_lut needs a lut_name parameter")
            staged = (lut_paths or {}).get(lut_name)
            if not staged:
                known = sorted(lut_paths or {})
                raise PlanError(
                    f"clip {clip_id!r}: LUT {lut_name!r} is not staged "
                    f"(staged: {known or 'none'} — resolve via creative.luts first)"
                )
            return (
                LutOp(
                    lut_name=lut_name,
        ...
        if twin == "subtitle":
            caption_asset_id = params.get("caption_asset_id")
            if not isinstance(caption_asset_id, str) or not caption_asset_id:
                raise PlanError(
                    f"clip {clip_id!r}: caption.burn_in needs a caption_asset_id parameter"
                )
            staged_sub = (subtitle_files or {}).get(caption_asset_id)
            if not staged_sub:
                known_subs = sorted(subtitle_files or {})
                raise PlanError(
                    f"clip {clip_id!r}: caption asset {caption_asset_id!r} has no staged "
                    f"subtitle file (staged: {known_subs or 'none'} — render SRT/ASS first)"
                )
            return (
                SubtitleOp(
                    subtitle_path=staged_sub,
        ...
        raise PlanError(
            f"clip {clip_id!r}: effect {operation!r} has invalid parameters: {exc}"
        ) from exc
    return None, operation
...
def compile_execution_plan(
    ...
    *,
    track_id: str,
    lut_paths: dict[str, str] | None = None,
    subtitle_files: dict[str, str] | None = None,
) -> ExecutionPlan:
    """Compile one track of the project timeline into a deterministic plan.

    ``lut_paths`` maps LUT names to staged ``.cube`` files and
    ``subtitle_files`` maps caption asset ids to staged SRT/ASS files; both
    are membership-checked (pure — no I/O) and a twin whose asset is missing
    raises :class:`PlanError` naming the clip.  The input project is never
    mutated: every segment owns freshly constructed ops (E1 isolation).
    """
    ...
    for clip in sorted(track.clips, key=lambda c: (c.timeline_range.start_us, c.clip_id)):
        ...
        for effect in clip.effects:
            lane_op, unmapped_name = _map_effect(
                effect, clip_id=clip.clip_id, lut_paths=lut_paths, subtitle_files=subtitle_files
            )
