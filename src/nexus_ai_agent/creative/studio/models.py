...
# ---------------------------------------------------------------------------
# Typed command envelope (protocol v1, envelope schema 1|2)
# ---------------------------------------------------------------------------
...
class TypedCommand(BaseModel):
    ...
    schema_version: Literal[1, 2] = LEGACY_SCHEMA_VERSION
    ...
    @model_validator(mode="after")
    def _schema2_requires_claims(self) -> TypedCommand:
        # A version bump must change validation, otherwise it is theater:
        # schema 2 is only meaningful with explicit claims attached.
        if self.schema_version == COMMAND_SCHEMA_VERSION:
            missing = [
                name
                for name, present in (
                    ("actor", self.actor is not None),
                    ("target.project_id", self.target.project_id is not None),
                    ("provenance", self.provenance is not None),
                )
                if not present
            ]
            if missing:
                raise ValueError("schema_version=2 requires explicit claims: " + ", ".join(missing))
        return self
