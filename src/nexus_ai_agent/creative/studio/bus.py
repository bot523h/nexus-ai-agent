...
The bus is a pure in-memory reducer. A trusted composition root supplies a
project authorizer; neither the command's actor nor its capability snapshot is
an authorization grant. A reservation exists *before* a handler runs and is
committed with its result under the same lock. Failures leave no reservation or
state change. This is NOT a durable cross-worker effects journal; the external
job/FFmpeg boundary lives above the bus.
"""
