...
class ReferenceExpr(BaseModel):
    ...
    def _validate_shape(self) -> ReferenceExpr:
        if self.kind == "absolute":
            if self.timecode_us is None or self.offset_us is not None:
                raise ValueError("kind='absolute' requires only timecode_us")
        elif self.kind == "relative":
            if self.offset_us is None or self.timecode_us is not None:
                raise ValueError("kind='relative' requires only offset_us")
        elif self.timecode_us is not None or self.offset_us is not None:
            raise ValueError(f"kind={self.kind!r} cannot carry a timecode or offset")
        return self
