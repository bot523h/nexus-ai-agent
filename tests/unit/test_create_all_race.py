"""Deterministic tests for the concurrent-``create_all`` retry (Phase D fix).
...
``table … already exists`` — which is what the concurrent ``nexus migrate``
integration test caught, and what made *every* racer fail instead of one.

The integration test can only observe the race statistically.  These tests pin
the fix deterministically, with a scripted engine that fails on demand:
